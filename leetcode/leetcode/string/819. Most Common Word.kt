...
fun mostCommonWord(paragraph: String, banned: Array<String>): String =
        paragraph
                .replace(regex = Regex("[^a-zA-Z0-9 ]"), replacement = " ")
                .split(" ")
                .filter { !banned.contains(it.toLowerCase()) && it.isNotBlank() }
...
fun main() {
    assertEquals(
            actual = mostCommonWord("Bob hit a ball, the hit BALL flew far after it was hit.", arrayOf("hit")),
            expected = "ball"
    )
    assertEquals(
            actual = mostCommonWord(
                    ...
                    banned = arrayOf("m", "i", "s", "w", "y", "d", "q", "l", "a", "p", "n", "t", "u", "b", "o", "e", "f", "g", "c", "x")),
            expected = "z"
    )
}